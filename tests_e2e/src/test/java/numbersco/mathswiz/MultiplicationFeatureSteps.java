package numbersco.mathswiz;

import cucumber.api.java.Before;
import cucumber.api.java.en.Given;
import cucumber.api.java.en.Then;
import numbersco.mathswiz.testutils.beans.AttemptResponse;
import numbersco.mathswiz.testutils.MultiplicationApplication;
...
public class MultiplicationFeatureSteps {
  ...
  public void the_user_sends_attempts(final String userAlias, final int attempts, final String rightOrWrong)
    ...
    int attemptsSent = IntStream.range(0, attempts)
        ...
        .peek(response -> lastAttemptResponse = response).mapToInt(response -> response.isCorrect() ? 1 : 0).sum();
